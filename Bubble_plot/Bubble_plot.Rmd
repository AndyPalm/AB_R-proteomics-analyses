...
date: "2026-02-05"
R_version: "4.4.2"
description: "Creates bubble plots to visualize protein abundance across multiple treatment groups. Each bubble represents a protein's mean intensity in a specific condition, with bubble size proportional to detection rate and color indicating Z-score or raw intensity."
---
...
```{r data_loading}
# --- DATA LOADING ---
# Read protein-level file, converting CSV to Parquet for efficient processing
...
```{r data_processing}
# --- DATA PROCESSING ---
# Complete the dataframe to handle missing replicates
...
# Calculate Z-Scores per protein across groups
df_agg <- df_agg %>%
  group_by(Protein) %>%
  mutate(
...
```{r data_processing_selection}
# --- DATA PROCESSING: Marker selection and sorting ---
# Identify the "Max Group" for each protein and select top N per group
...
# Select Top N proteins per group
top_n_per_group <- 15 
...
# Filter Data & Apply Aesthetic Ordering
plot_data <- df_agg %>%
...
# Note: common_theme is a candidate for extraction to src/functions.R as theme_custom()
...
```{r data_processing_subcell}
# --- DATA PROCESSING: PM/surface-annotated protein subset ---
...
protein_spread_ranking <- df_agg_subcell %>%
  filter(GROUP %in% target_groups) %>%
  filter(is.na(Gene) | !str_detect(Gene, "^KRT")) %>%
  group_by(Protein) %>%
  summarise(
...
# Filter plot data to selected subcell proteins
plot_data_subcell <- df_agg_subcell %>%
  filter(Protein %in% selected_proteins_subcell$Protein)
```

```{r visualization_subcell}
# --- VISUALIZATION: Subcell subset bubble plot ---
# Set Column Order
col_order <- c("ALOD4DF", "OlyADF", "ALOD4UF", "OlyAUF", "DF", "UF")
...
plot_data_subcell <- plot_data_subcell %>%
  mutate(
    Z_Bin   = factor(cut(ZScore, breaks = z_breaks, labels = z_labels), levels = z_labels),
    Raw_Bin = factor(cut(MeanIntensity, breaks = raw_breaks, labels = raw_labels), levels = raw_labels)
  )
...
p3 <- ggplot(plot_data_subcell, aes(x = GROUP, y = Protein)) +
  ...
  labs(title = "Protein Expression (Z-Score) — Subcell List", x = "", y = "") +
  common_theme +
  coord_fixed(ratio = 0.5)
...
```{r export_subcell}
# --- EXPORT: Save subcell bubble plot ---
