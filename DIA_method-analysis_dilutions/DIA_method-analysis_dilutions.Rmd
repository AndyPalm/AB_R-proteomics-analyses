...
date: "2025-10-09"
R_version: "4.4.2"
description: "Evaluates DIA quantification accuracy across dilution series. Compares expected vs. observed fold changes across different feature subsets and processing methods (narrow vs. wide isolation windows)."
---
...
```{r data_loading}
# --- DATA LOADING: Setup paths and helper functions ---
...
# Helper function to give a logical ordering of groups (not alphabetical)
order_featureSubset <- function(x){
  factor(x, levels = c("highQuality", "top3", "top4", "top5", "top10", "top50"))
}
...
```{r data_cleaning}
# --- DATA CLEANING: Split and harmonize tables ---
# Split by table type
...
```{r analysis}
# --- ANALYSIS: Coverage, FC error, imputation burden, dilution calibration ---
...
write_csv(calib, file.path(out_dir, "dilution_calibration_by_protein.csv"))
...
disc <- inner_join(narr_10, wide_10,
  ...
  ungroup() %>%
  mutate(
    delta_trans = int_wide - int_narrow,
    delta_pep = pep_med_wide - pep_med_narrow,
    disc_score = delta_trans - delta_pep
  )
...
```{r analysis_intra_method}
# --- ANALYSIS: Intra-Method Discordance via Dilution Response ---
trans_dil <- feat %>%
...
trans_fc <- trans_wide %>%
  mutate(
    fc_5v1_obs = int_1 - int_5,
    fc_10v5_obs = int_5 - int_10,
    fc_10v1_obs = int_1 - int_10,
    fc_5v1_exp = log2(5),
    fc_10v5_exp = log2(2),
    fc_10v1_exp = log2(10),
...
trans_discord <- trans_fc %>%
  ...
  mutate(
    ...
    is_discord_10x = abs(discord_10v1) > 1.0
  )
