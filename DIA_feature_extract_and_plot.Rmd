...
```{r}
extract_list <- c("EGFR")  ## list your proteins here!!
...
Extracting features
```{r}
# 1. Clean the dataframe (The Surgical Fix)
...
if (nrow(features_subset) == 0) {
  warning("No rows returned. Check if 'EGFR' matches the casing in your 'Gene' column exactly.")
} else {
...
source(here::here("src", "DIA_feature_extract_boxwhisker.R")) 
