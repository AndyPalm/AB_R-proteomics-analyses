...
date: "2026-01-16"
R_version: "4.4.2"
description: "Multi-omics integration of transcriptomic and proteomic datasets. Identifies discordant regulation where protein abundance diverges from transcript levels, implying post-transcriptional regulation. Uses Robust Linear Models (RLM) to calculate discordance residuals."
---

```{r setup, include=FALSE}
# --- SETUP: Load required libraries and define global paths/constants ---
library(arrow)
library(ggbeeswarm)
library(ggpubr)
library(ggrepel)
...
# Protein/gene labels to highlight in plots
...
```{r data_loading}
# --- DATA LOADING ---
# Helper function: CSV to Parquet conversion (candidate for extraction to src/functions.R)
...
```{r data_processing_rna}
# --- DATA PROCESSING: RNA ---
# Pivot long, calculate stats, and categorize genes
...
# Calculate Stats & Categorize (Robust Logic)
...
```{r data_processing_protein}
# --- DATA PROCESSING: Protein ---
# 1. Filter out 'static' and tag conditions explicitly
prot_data <- exp.protein_input %>%
  filter(GROUP != "static") %>% # Removes 'static' rows immediately
  mutate(Condition = case_when(
    GROUP == cond1 ~ cond1,
    GROUP == cond2 ~ cond2,
    TRUE ~ "Other"
  )) %>%
  filter(Condition != "Other") # Ensure only DF and UF remain

# 2. Calculate counts per group for binning
...
# 3. Quantitative Stats (Requires 2+ replicates in BOTH conditions)
valid_prots <- prot_counts %>% 
...
# 4. Exclusive Lists (Singletons)
list_only_c1 <- prot_counts %>% filter(n_cond1 >= 2 & n_cond2 == 0) %>% pull(Protein)
list_only_c2 <- prot_counts %>% filter(n_cond2 >= 2 & n_cond1 == 0) %>% pull(Protein)
...
# --- DATA PROCESSING: Tiered merge of protein and RNA data ---
...
# Combine & Discordance Calculation
proteogenomics <- bind_rows(match_tier1, match_tier2)

# Robust Linear Model (M-weighting reduces influence of outliers)
...
```{r visualization_regression}
# --- VISUALIZATION: Studentized residuals analysis (Robust Linear Model) ---
# M-weighting model reduces influence of outliers on fitted linear model.
# Useful for heteroscedastic datasets (variance not constant across all values).
...
p_resid <- ggplot(plot_df, aes(x = RNA_Log2FC, y = Protein_Log2FC)) +
  # Base Points
...
p_resid
# ggsave(filename = file.path(output_dir, "plot-studentized-residuals_2Mar26.pdf"), plot = p_resid, width = 7, height = 7)
```
```{r}
# --- 1. AXIS CUSTOMIZATION SETTINGS ---
# Define your desired range and intervals here
z_min  <- -8
z_max  <- 8
z_step <- 2
# #
# Generate the vector of breaks based on your settings
z_breaks <- seq(z_min, z_max, by = z_step)
z_lims   <- c(z_min - 0.5, z_max + 0.5)
# #
# --- 2. UPDATED MAIN Z-SCORE PLOT ---
# Calculate discordance based on Z-score difference
plot_df_z <- proteogenomics_stats %>%
  mutate(
    Z_Diff = Z_Protein - Z_RNA,
    Color_Z = case_when(
      Z_Diff > 3  ~ paste("High in", cond1),
      Z_Diff < -3 ~ paste("High in", cond2),
      TRUE        ~ "Concordant"
    )
  )
# #
p_zscore <- ggplot(plot_df_z, aes(x = Z_RNA, y = Z_Protein)) +
  geom_abline(slope = 1, intercept = 0, color = "black", size = 1) +
  geom_abline(slope = 1, intercept = 3, linetype = "dashed", color = "grey40") +
  geom_abline(slope = 1, intercept = -3, linetype = "dashed", color = "grey40") +
  geom_point(aes(color = Color_Z), alpha = 0.7) +
  geom_point(data = subset(plot_df_z, `Gene Names (primary)` %in% plot_labels),
             shape = 1, size = 1.9, color = "black", stroke = 0.8) +
  geom_text_repel(data = subset(plot_df_z, `Gene Names (primary)` %in% plot_labels),
                  aes(label = `Gene Names (primary)`), 
                  size = 5, fontface = "plain", box.padding = 0.5, max.overlaps = Inf) +
  scale_color_manual(values = my_colors_z) +
  # Custom Axis Breaks and Labels
  scale_x_continuous(breaks = z_breaks) + 
  scale_y_continuous(breaks = z_breaks) +
  coord_cartesian(xlim = z_lims, ylim = z_lims) + 
  theme_classic() +
  theme(
  ...
  labs(
    title = "Z-Score Discordance Analysis",
    x = expression(bold(RNA~Z-Score)),
    y = expression(bold(Protein~Z-Score)),
    color = "Discordance Status"
  )
# #
# --- 3. PREPARE SINGLETON Z-SCORE DATA ---
# We calculate Z-scores for intensities within the singleton populations
# #
# Protein Singletons
df_prot_c1 <- protein_only_cond1 %>%
  ...
  mutate(Z_Intensity = as.numeric(scale(mean_intensity)))
# #
...
df_rna_c1 <- stats.rna.clean %>%
  ...
  mutate(mean_val = log2(Mean_cond1 + 1), Label = Gene) %>%
  mutate(Z_Intensity = as.numeric(scale(mean_val)))
# #
df_rna_c2 <- stats.rna.clean %>%
  filter(Category == paste("Only in", cond2)) %>%
  mutate(mean_val = log2(Mean_cond2 + 1), Label = Gene) %>%
  mutate(Z_Intensity = as.numeric(scale(mean_val)))
# #
# --- 4. GENERATE GUTTER PLOTS ---
# Using the same logic but targeting the Z_Intensity column
# #
p_prot_1_z <- generate_horizontal_gutter(
  df_prot_c1 %>% mutate(mean_intensity = Z_Intensity), 
  c1_color, paste("Protein:", cond1),
  x_limits = z_lims, axis_label = "Intensity Z-Score",
  axis_side = "bottom", label_side = "top"
) + scale_x_continuous(breaks = z_breaks, limits = z_lims)
# #
p_prot_2_z <- generate_horizontal_gutter(
  df_prot_c2 %>% mutate(mean_intensity = Z_Intensity), 
  c2_color, paste("Protein:", cond2),
  x_limits = z_lims, axis_label = "Intensity Z-Score",
  axis_side = "top", label_side = "bottom"
) + scale_x_continuous(breaks = z_breaks, limits = z_lims)
# #
p_rna_1_z  <- generate_vertical_gutter(
  df_rna_c1 %>% mutate(mean_intensity = Z_Intensity), 
  c1_color, paste("RNA:", cond1),
  y_limits = z_lims, axis_label = "Intensity Z-Score",
  axis_side = "left", label_side = "right"
) + scale_y_continuous(breaks = z_breaks, limits = z_lims)
# #
p_rna_2_z  <- generate_vertical_gutter(
  df_rna_c2 %>% mutate(mean_intensity = Z_Intensity), 
  c2_color, paste("RNA:", cond2),
  y_limits = z_lims, axis_label = "Intensity Z-Score",
...
#ggsave(filename = file.path(output_dir, "plot-zscore_2Mar26.pdf"), plot = p_zscore, width = 7, height = 7)
#ggsave(filename = file.path(output_dir, "z-score-protein-only-DF_2mar26.pdf"), plot = p_prot_1_z, width = 7, height = 3)
#ggsave(filename = file.path(output_dir, "z-scoreprotein-only-UF_2mar26.pdf"), plot = p_prot_2_z, width = 7, height = 3)
#ggsave(filename = file.path(output_dir, "z-score-RNA-only-DF_2mar26.pdf"), plot = p_rna_1_z, width = 3, height = 7)
#ggsave(filename = file.path(output_dir, "z-score-RNA-only-UF_2mar26.pdf"), plot = p_rna_2_z, width = 3, height = 7)
```


```{r visualization_gutter}
# --- VISUALIZATION: Gutter plots for condition-exclusive proteins ---
STRIP_RATIO <- 8  

# Prepare Protein Data (aggregated by Protein to ensure single points)
df_prot_c1 <- protein_only_cond1 %>%
  left_join(df.labels %>% select(Entry, `Gene Names (primary)`), by = c("Protein" = "Entry")) %>%
  mutate(Label = coalesce(`Gene Names (primary)`, Protein)) %>%
  group_by(Protein, Label) %>%
...
# Horizontal Plot (used for Proteins)
...
#ggsave(filename = file.path(output_dir, "protein-only-DF_2mar26.pdf"), plot = p_prot_1, width = 7, height = 3)
#ggsave(filename = file.path(output_dir, "protein-only-UF_2mar26.pdf"), plot = p_prot_2, width = 7, height = 3)
#ggsave(filename = file.path(output_dir, "RNA-only-DF_2mar26.pdf"), plot = p_rna_1, width = 3, height = 7)
#ggsave(filename = file.path(output_dir, "RNA-only-UF_2mar26.pdf"), plot = p_rna_2, width = 3, height = 7)
```

```{r analysis_halflife}
# --- ANALYSIS: Protein half-life analysis ---
# From Savitski's 2018 Nat. Comm paper (proteome-wide half-life determination)
# Half-lives condensed to a single value averaged across all cell lines where data quality = "good"
...
# Merge (Primary + Secondary Rescue)
...
```{r visualization_halflife}
# --- VISUALIZATION: Protein half-life histogram ---
overflow_threshold <- 400

half_life_data_cleaned <- proteogenomics_protein_half_lives %>%
  filter(!is.na(final_half_life)) %>%
  mutate(
    half_life_capped = pmin(final_half_life, overflow_threshold)
  )
...
```{r analysis_ubiquitylation}
# --- ANALYSIS: Ubiquitylation enrichment analysis ---
# Uses ubiquitylation data from Choudhary 2024 Cell paper
# Creates binary TRUE/FALSE for whether protein is ubiquitinated
# Compares between discordant/concordant groups
